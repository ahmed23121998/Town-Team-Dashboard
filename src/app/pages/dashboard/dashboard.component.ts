...
import { Firestore, collectionGroup, onSnapshot, query, collection, getDocs } from '@angular/fire/firestore';
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  private async loadDashboardData() {
    ...
    for (const order of orders) {
      const cartItems = Array.isArray((order as any)['cartItems']) ? (order as any)['cartItems'] : [];
      let orderTotal = 0;
      for (const item of cartItems) {
        orderTotal += (item.price || item.unitPrice || 0) * (item.quantity || 1);
        // category
        let cat = item.category;
        if (!cat || cat.trim() === '' || cat.toLowerCase() === 'unknown') cat = 'Other';
        categoriesCount[cat] = (categoriesCount[cat] || 0) + (item.quantity || 1);
      }
      ...
      if (cartItems.length > 0) {
        recentOrders.push({
          id: order.id,
          date: orderDate?.toDate ? orderDate.toDate() : new Date(orderDate),
          productImage: cartItems[0].image || 'assets/no-image.png',
          productName: cartItems[0].title || 'No Name',
          customer: (order as any).email || (order as any).city || 'Customer',
    ...
    // ترتيب آخر الطلبات
    this.recentOrders = recentOrders.sort((a, b) => b.date.getTime() - a.date.getTime()).slice(0, 8);

    // Top Categories
    let filteredCategories = Object.entries(categoriesCount).filter(([cat]) => cat !== 'Other' && cat.toLowerCase() !== 'unknown');
    if (filteredCategories.length === 0) filteredCategories = Object.entries(categoriesCount);
    ...
    const topCatColors = ['#3B82F6', '#14B8A6', '#F97316', '#FACC15', '#A855F7', '#FF6B6B', '#FFD166', '#06D6A0', '#118AB2', '#EF476F'];
    ...
    // آخر 5 أوردرات فقط
    this.recentOrders = recentOrders.sort((a, b) => b.date.getTime() - a.date.getTime()).slice(0, 5);
  }
